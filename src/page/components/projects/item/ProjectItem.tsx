...
export const ProjectItem: FC<IProjectItem> = ({
  title,
  shortDescription,
  gitHubLink,
  ...rest
  ...
  const { ref, isIntersecting } = useIntersect({
    options: { rootMargin: "150px 0px -50px 0px" },
  });

  return (
    <div
      className={`${isIntersecting && styles.visible} ${styles.container}`}
      ref={ref}
    >
      <div {...{ ...rest }} tabIndex={0} className={`${styles.wrapper}`}>
